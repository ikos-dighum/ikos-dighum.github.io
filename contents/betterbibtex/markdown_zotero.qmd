...
Then open Zotero. If you have a library, fine. If not, we will make a small one now.
...
-   If this is an ongoing project and you know your Zotero library is likely to expand, choose the "keep updated" option. This ensures that Zotero sends an updated bibliography file to your project folder every time you make a change.
-   Now comes the part where you choose where to export the file. *Choose the exact folder where your new .md file is located.*
-   You now have a .bib file where the file name is the same as the name of the Zotero collection you exported. Go and check if it is in the right place.
...
When it is compiled to .html format, it will look like the paragraph below, and a list of references will be added at the bottom of the document. That's the magic!
...
Now try it out with different file formats. For pdf to work, you must have installed a Latex package (tinytex or Texlive). If you have non-Latin characters in your .bib file you may run into a couple of problems that we can solve together.
